    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger asChild>
        {renderTrigger ? (
          ...
          <div
            className={cn(
              "flex w-full cursor-pointer items-center h-10 ps-3 pe-1 font-normal rounded-md text-sm shadow-sm border bg-background border-border",
      ...
      <PopoverContent className="w-auto p-2">
        ...
        <div className="relative overflow-hidden">
          <Calendar
            ...
            disabled={(d) => !!((max && d > max) || (min && d < min))}
...
function TimePicker({
  ...
  const onMinuteChange = useCallback((v: TimeOption) => {
    isUserChange.current = true;
    setMinute(v.value);
  }, []);
