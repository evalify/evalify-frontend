...
import { useAddQuestionsToQuiz } from "@/components/bank/quiztobank/hooks/use-bank-operations";
...
interface StepData {
  selectedBank?: Bank;
  availableQuestions?: RepoBankQuestion[]; // All questions fetched from filters
  selectedQuestions?: RepoBankQuestion[]; // User's final selection
  addedCount?: number;
}
...
export function MultiStepBankModal({
  ...
  const addQuestionsToQuizMutation = useAddQuestionsToQuiz();
  ...
  const handleQuestionsFiltered = useCallback(
    (questions: RepoBankQuestion[]) => {
      // Store available questions and initially select all
      setStepData((prev) => ({
        ...prev,
        availableQuestions: questions,
        selectedQuestions: questions, // Initially select all
      }));
    },
    [],
  );

  const handleQuestionsSelectionChange = useCallback(
    (questions: RepoBankQuestion[]) => {
      // Update the selected questions with the user's final selection
      setStepData((prev) => ({
        ...prev,
  ...
  const handleAddQuestions = useCallback(
    (selectedQuestions: RepoBankQuestion[]) => {
      ...
      const dto: AddBankQuestionToQuizRequest = {
        sectionId: sectionId,
        bankQuestionId: backendQuestionIds,
      };

      addQuestionsToQuizMutation.mutate(
        { quizId, request: dto },
        {
          onSuccess: (response) => {
            setStepData((prev) => ({
    ...
    [sectionId, addQuestionsToQuizMutation, error, quizId, handleNext],
  );
  ...
  const canProceed = useMemo(() => {
    switch (currentStep.id) {
      ...
      case "filters":
        return !!stepData.availableQuestions?.length; // Can proceed when questions are available
      case "questions":
      ...
      case "filters":
        return (
          <FilterSelectionComponent
            ...
            quizId={quizId}
            sectionId={sectionId}
            onQuestionsFiltered={handleQuestionsFiltered}
      ...
      case "questions":
        return (
          <QuestionSelectionComponent
            questions={stepData.availableQuestions || []}
            onQuestionsSelected={handleQuestionsSelectionChange}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-5xl md:max-w-6xl max-w-[95vw] max-h-[98vh] p-0 overflow-hidden w-full h-[98vh] flex flex-col">
        {/* Header */}
              ...
              <div className="flex items-center gap-2">
                ...
                {stepData.selectedQuestions &&
                  stepData.selectedQuestions.length > 0 && (
                    ...
                    >
                      {stepData.selectedQuestions.length} question
                      {stepData.selectedQuestions.length !== 1 ? "s" : ""}{" "}
                      selected
                    </Badge>
                  )}
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {addQuestionsToQuizMutation.isPending ? (
                      ...
                      <>
                        <CheckCircle className="h-4 w-4" />
                        Add {stepData.selectedQuestions?.length || 0} Question
                        {(stepData.selectedQuestions?.length || 0) !== 1
                          ? "s"
                          : ""}
                        to Quiz
                      </>
